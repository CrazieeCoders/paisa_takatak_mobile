...
import 'package:paisa_takatak_mobile/bloc/loan_confirmation_bloc/loan_confirmation_bloc.dart';
import 'package:paisa_takatak_mobile/bloc/loan_confirmation_bloc/loan_confirmation_event.dart';
...
class LoanConfirmationProvider extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return BlocProvider<LoanConfirmationBloc>(
        create: (context) => LoanConfirmationBloc(), child: LoanConfirmation());
  }
}
...
class _LoanConfirmationState extends State<LoanConfirmation> {
  ...
  LoanConfirmationBloc confirmationBloc;

  @override
  Widget build(BuildContext context) {
    confirmationBloc = BlocProvider.of<LoanConfirmationBloc>(context);
    ...
    return Scaffold(
      body: MultiBlocListener(
        listeners: [
          ...
          BlocListener<LoanConfirmationBloc,LoanConfirmationState>(
              listener:  (context, state){

                if(state is LoanConfirmationLoading){
                  LoadingDialog.show(context);
                }else if(state is LoanConfirmationFailure){
                  LoadingDialog.hide(context);
                  Fluttertoast.showToast(msg: "Failed to update server please contact with admin");
                }else if(state is LoanConfirmationSuccess){
                  LoadingDialog.hide(context);
        ...
        child: Stack(
          children: [
            ...
            Padding(
              padding:
                  EdgeInsets.fromLTRB(3.89 * w, 7.31 * h, 3.89 * w, 1.95 * h),
              child: Container(
                decoration: BoxDecoration(
                  ...
                  children: [
                    ...
                    Center(
                      child: Container(
                          ...
                          width: 68.61 * w,
                          child: Image.asset(
                              'cutouts/LoanApplyConfirmation/confirm-icon@2x.png')),
                    ),
                    SizedBox(
                      height: 2.43 * h,
                    ),
                    ...
                    Center(
                        child: Text(
                      'We\' ll notify about your loan approval',
                    ...
                    Center(
                      child: Row(
                        ...
                        children: [
                          GestureDetector(
                            onTap: () {
                              confirmationBloc.add(LoanConfirmationPressed());
